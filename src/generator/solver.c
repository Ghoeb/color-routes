...
bool zone_is_solvable(Zone* zone)
{
    ...
    {
        Client* client = zone -> buildings[i];

        /* Para cada cliente sin pareja */
        if(!city_client_is_taken(client))
        {
            if(city_client_is_blank(client))
            {
                if(client -> zone == chain_end(client) -> zone)
                {
                    loop = true;
                }
                else
...
bool recursively_check(Client* client)
{
    bool ret = true;
    Zone* zone = client -> zone;
    if(!zone -> checked)
    {
        ret = zone_is_solvable(zone);
        zone -> checked = true;
        if(!ret) return false;
        for(int i = 0; i < client -> link_count; i++)
        {
            ret = ret && recursively_check(client -> linked[i]);
        }
    }
    return ret;
}
...
bool assign_next(Client* next, Choice* steps)
{
    ...
    if(next -> color != val -> color)
    {
        ...
        if(next -> color)
        {
            ...
            choice -> tinted = val;
            // ret = client_paint_check_zone(val, next -> color /*, val -> abastecedor */);
            client_paint(val, next -> color);
        }
        else
        {
            /* Sino, pintamos al que tomo la decision */
            choice -> tinted = next;
            client_paint(next, val -> color);
            // ret = client_paint_check_zone(next, val -> color /*, val -> abastecedor */);
        }
        choice -> tinted -> zone -> checked = true;

        ret = recursively_check(choice -> tinted -> linked[0]);
    }
    ...
    else if(!next -> color)
    {
        ...
        Client* end1 = chain_end(next);
        Client* end2 = chain_end(val);

        if(end1 -> zone == end2 -> zone)
        {
            ret = false;
        }

        /* PODA 4: Si dejamos un par de ciclos inresolvibles */

        if(!zone_is_solvable(end1 -> zone) || !zone_is_solvable(end2 -> zone))
        {
            ret = false;
        }
    }
...
bool options(Client* client)
{
    ...
    for (int i = 0; i < zone -> building_count; i++)
    {
        ...
        if(is_legal(client,pal))
        {
            ...
            // }
            heap_insert(client -> options, pal, rand());
        }
    }
...
Client* find_unnasigned_location(Zone** to_solve, size_t count)
{
    ...
    {
        for(int j = 0; j < to_solve[i] -> building_count; j++)
        {
            Client* challenger = to_solve[i] -> buildings[j];
...
/** Marca las zonas alcanzables desde esta como de un mismo grupo */
void expand_group(Zone* zone, size_t group)
{
    if(!zone -> core && !zone -> group)
    {
        zone -> group = group;

        for(int i = 0; i < zone -> building_count; i++)
        {
            expand_group(zone -> buildings[i] -> vecino, group);
        }
    }
}
...
size_t solver_init(Layout* layout)
{
...
bool sub_solve(Zone** to_solve, size_t count, Choice* steps)
{
    do
    {
        /* Obtenemos el siguiente cliente a asignar */
        Client* next = find_unnasigned_location(to_solve, count);
...
{
    size_t groups = solver_init(layout);

    Zone** to_solve = malloc(sizeof(Zone*) * layout -> zone_count);
    size_t to_solve_count = 0;


    for(size_t i = 1; i <= groups; i++)
    {
        to_solve_count = 0;
        for(int j = 0; j < layout -> zone_count; j++)
        {
            Zone* zone = layout -> zones[j];
            if(zone -> group == i)
            {
                to_solve[to_solve_count++] = zone;
            }
        }

        fprintf(stderr, "Solving group %zu\n", i);

        /* Si un sub-problema es inresolvible, entonces es inresolvible */
        if(!sub_solve(to_solve, to_solve_count, steps))
        {
